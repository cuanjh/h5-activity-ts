...
class Activity {
  ...
  initEvent() {
    ...
    this.eleBtnDraw.addEventListener('click', (e) => {
      ...
      if (this.drawNum > 0) {
        ...
        this.showTips('You\'ve run out the chance this week.');
      }
    })
  ...
  rewardDesc(iterator) {
    ...
    switch (iterator.type) {
      case VoucherType.Discount:
        desc = `${100 - iterator['money_usd'] * 10}% off`
        break;
      case VoucherType.Cash:
        desc = `$${iterator['money_usd']}`
        break;
      default:
  ...
  async initRewardInfo() {
    ...
    for (const iterator of res['recordsList']) {
      pTag += `
      <p>
        <span>Congratulation ${iterator.talkmateId} won</span>
        <span>${iterator.title_en}</span>
      </p>`;
    }
